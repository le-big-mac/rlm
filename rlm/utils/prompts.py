...
4. A `rlm_query(prompt, model=None, budget=None)` function that spawns a **recursive RLM sub-call** for deeper thinking subtasks. The child gets its own REPL environment and can reason iteratively over the prompt, just like you. Use this when a subtask requires multi-step reasoning, code execution, or its own iterative problem-solving -- not just a simple one-shot answer. Falls back to `llm_query` if recursion is not available. `budget` (float, USD) optionally caps the child's spending.
5. A `rlm_query_batched(prompts, model=None, budgets=None)` function that spawns multiple recursive RLM sub-calls concurrently. Each prompt gets its own child RLM. `budgets` (list of floats, USD) optionally caps each child's spending -- useful for allocating more budget to harder subtasks. If omitted, remaining budget is split equally. Falls back to `llm_query_batched` if recursion is not available.
